...
net
  .createServer((client) => {
    const handler = (data) => client.write(JSON.stringify(data) + "\0"); //A null byte '\0' is inserted between messages.
    actors.add(handler);
    console.log("actor pool connected", actors.size);
    client
      .on("end", () => {
        actors.delete(handler); //When a client connection is closed, it is removed from the actors set.
        console.log("actor pool disconnected", actors.size);
      })
      .on("data", (raw_data) => {
        const chunks = String(raw_data).split("\0"); //The data events may contain multiple messages and are split on null bytes.
        chunks.pop(); //The final byte is a null byte, so the last entry in chunks is an empty string.
