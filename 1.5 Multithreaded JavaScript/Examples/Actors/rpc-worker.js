...
module.exports = class RpcWorkerPool {
  constructor(path, size = 0, strategy = "roundrobin") {
    if (size === 0)
      this.size = CORES; //The thread pool size is highly configurable.
    else if (size < 0) this.size = Math.max(CORES + size, 1);
    else this.size = size;
    if (!STRATEGIES.has(strategy)) throw new TypeError("invalid strategy");
    this.strategy = strategy; //The strategy is validated and stored.
    this.rr_index = -1;
    this.next_command_id = 0;
    this.workers = []; //An array of workers is maintained instead of just one.
    for (let i = 0; i < this.size; i++) {
      const worker = new Worker(path);
      this.workers.push({ worker, in_flight_commands: new Map() }); //The in_flight_commands list is now maintained per worker.
  ...
  exec(method, ...args) {
    ...
    const worker = this.getWorker(); //The applicable worker is looked up.
